      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-slate-200">
        ...
        {prediction && (
          <div className="bg-emerald-50 p-6 border-t border-emerald-100">
            ...
            {/* CANVAS: Buscamos dentro de la doble caja (prediction.prediction) */}
            {prediction.prediction && prediction.prediction.main_numbers && Array.isArray(prediction.prediction.main_numbers) ? (
              <div className="flex flex-col items-center gap-5">
                ...
                <div className="flex flex-wrap justify-center gap-3">
                  {/* Esferas Negras (Números Principales) */}
                  {prediction.prediction.main_numbers.map((num: number, idx: number) => (
                    <div key={idx} className="w-12 h-12 rounded-full bg-slate-900 text-white flex items-center justify-center text-xl font-bold shadow-lg border-2 border-slate-700 transform transition-transform hover:scale-110">
                      {num}
                    </div>
                  ))}

                  {/* Esfera Esmeralda (Super Balota - Opcional) */}
                  {prediction.prediction.special_number !== null && prediction.prediction.special_number !== undefined && (
                    <div className="w-12 h-12 rounded-full bg-emerald-500 text-white flex items-center justify-center text-xl font-bold shadow-lg border-2 border-emerald-400 ml-2 transform transition-transform hover:scale-110">
                      {prediction.prediction.special_number}
                    </div>
                  )}
                </div>

                {/* Píldora de Serie (Para Loterías Tradicionales como Cundinamarca) */}
                {prediction.prediction.serie && (
                  <div className="bg-slate-200 text-slate-700 px-5 py-1.5 rounded-full text-sm font-bold border border-slate-300 shadow-inner">
            ...
            <p className="text-center text-xs text-emerald-600 mt-5 opacity-70 font-medium">
